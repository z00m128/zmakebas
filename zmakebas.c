 ...
 */
  ...
  do {
     ...
     */
      ...
      if ((ptr = strstr(lcasebuf, "rem")) != NULL && !isalpha(ptr[-1]) && !isalpha(ptr[3])) {
      ...
      for (tarrptr = tokens; *tarrptr != NULL; tarrptr++) {
         ...
         * <>, <=, >=.
         */
          if ((*tarrptr)[0] == '<' || (*tarrptr)[1] == '=' || (!isalpha(ptr[-1]) && !isalpha(ptr[toklen]))) {
            ptr2 = linestart + (ptr - lcasebuf);
       ...
       */
         ...
         */
          ...
          for (f = 0; f < labelend; f++) {
            ...
            if (memcmp(labels[f], ptr, len) == 0 && (ptr[len] < 33 || ptr[len] > 126 || ptr[len] == ':')) {
       ...
       */
        if (!in_rem && !in_quotes && !isalpha(ptr[-1]) && (isdigit(*ptr) || ((*ptr == '-' || *ptr == '+' || *ptr == '.') && isdigit(ptr[1])) || ((*ptr == '-' || *ptr == '+') && ptr[1] == '.' && isdigit(ptr[2])))) {
